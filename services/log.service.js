...
const printHelp = () => {
	console.log(
		...
		при задании ${chalk.yellow('h1, ...')} разобъет суммирование энергию по группам до и после указанных часов, 
		...
		${chalk.blue('Используемые файлы, хранятся в рабочей директории:')}
		${chalk.yellow('power.txt')} - файл с профилем мощности, вручную снимаем показания со сетчика электрической энергии СЭТ-4М, 
		            переименовываем и сохроняем у себя в рабочей директории
		${chalk.yellow(`power-report.json`)} - создается автоматически при вводе общих данных и сохраняет их для последующего использования 
		${chalk.yellow(`report.xlsx`)} - сгенерированный отчет по электрической энергии `);
...
const printGuide = () => {
	console.log(
		...
		${chalk.red('В ОС Windows может быть отключено отображение расширений, тогда переименовываем просто в power')}
